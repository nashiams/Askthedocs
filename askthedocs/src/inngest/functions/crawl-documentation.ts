...
import { extractCodeFromHtml, extractCodeFromMarkdown } from "./lib/manual-crawler";
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        const snippets = await step.run(`extract-page-${i}`, async () => {
          try {
            ...
            // Check if content is markdown (from Firecrawl) or HTML
            if (result.content.includes('```') && result.method === 'firecrawl') {
              // Likely markdown from Firecrawl
